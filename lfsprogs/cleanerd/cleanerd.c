 ...
 */
int lfs_markv_emul(int fd, BLOCK_INFO *blkiov, int blkcnt)
{
	struct lfs_ioc li;
	printf("lfs_markv_emul called\n");
	
	li.bip = blkiov;
	li.nblocks = blkcnt;
	return ioctl(fd, LFS_MARK, &li);
}

int lfs_bmapv_emul(int fd, BLOCK_INFO *blkiov, int blkcnt)
{	struct lfs_ioc li;

	printf("lfs_bmapv_emul called\n");
	li.bip = blkiov;
	li.nblocks = blkcnt;
	return ioctl(fd, LFS_BMAP, &li);
}

int lfs_segwait_emul(int fd, struct timeval *tv)
{
	printf("lfs_segwait_emul called\n");
#if 0
...
unsigned long cost_benefit(struct fs_info *fsp, struct segusage *su)
{
...
main(int argc, char **argv)
{
	...
	if (!(mntname = fs_getmntinfo(fs_name, "lfs"))) 
		log_exit(0, LOG_ERR, "%s is not mounted or does not contain LFS\n", fs_name);
	...
	for(fsp = get_fs_info(fs_name, mntname, do_mmap); ;
		  reread_fs_info(fsp, do_mmap)) {
		ifile_fd = get_ifile_fd(fsp);
		/*
		...
		if (clean_loop(fsp, segs_per_clean, clean_opts))
			continue;
#endif
		clean_loop(fsp, segs_per_clean, clean_opts);
		break;
...
int invalidate_segment(FS_INFO *fsp, int sn)
{
	struct seglist sl;
	SEGS_AND_BLOCKS sb;
...
int clean_loop(struct fs_info *fsp, int nsegs, long options)
{

	time_t	now;
	struct lfs_super_block *lfsp;
	unsigned long max_free_segs;
	unsigned long fsb_per_seg;

	lfsp = &fsp->fi_lfs;
	max_free_segs = MAX(fsp->fi_cip.bfree, 0) / seg_size();
	
	now = time((time_t *)NULL);

	printf("max_free_segs = %lu \n", max_free_segs);
	if (debug > 2 || (fsp->fi_cip.clean < max_free_segs &&
	     (fsp->fi_cip.clean <= lfsp->s_minfreeseg ||
...
void clean_fs(FS_INFO *fsp, unsigned long (*cost_func)(FS_INFO *, SEGUSE *),
	 int nsegs, long options)
{
	...
	if ((segs =
	    malloc(fsp->fi_lfs.s_nseg * sizeof(struct seglist))) == NULL) {
...
#if 0
	/* If we really need to clean a lot, do it now */
	...
	if (i) {
		...
		if (options & CLEAN_BYTES) {
			 ...
			 */
			cleaned_bytes = 0;
			to_clean = nsegs * seg_size() - seg_size() / 4;
...
choose_segments(FS_INFO *fsp, struct seglist *seglist, unsigned long (*cost_func)(FS_INFO *, SEGUSE *))
{
	...
	for (sp = seglist, i = 0; i < lfsp->s_nseg; ++i) {
		if (debug > 1) {
			printf("%d...", i);
			fflush(stdout);
		}
		sup = fsp->fi_segusep + i;
		PRINT_SEGUSE(sup, i);
	...
	if (debug > 2)
		for(i = 0; i < nsegs; i++) {
			syslog(LOG_DEBUG, "segment %lu age %lu"
			       " contains %lu priority %lu\n",
			       seglist[i].sl_id, seglist[i].sl_age,
 ...
 */
int add_segment(FS_INFO *fsp, struct seglist *slp, SEGS_AND_BLOCKS *sbp)
{
	int id = slp->sl_id;
	BLOCK_INFO *tba, *_bip;
	SEGUSE *sp;
	...
	lfsp = &fsp->fi_lfs;
	sp = fsp->fi_segusep + id;
	seg_addr = seg_size() * id + LFS_SEGSTART * LFS_BSIZE;
	error = 0;
	tba = NULL;
	...
	segl = (struct seglist **)realloc(sbp->segs,
	    sizeof(struct seglist *) * (sbp->nsegs + 1));
	buf = (caddr_t *)realloc(sbp->buf, sizeof(caddr_t) * (sbp->nsegs + 1));
	if (!segl || !buf)
		err(1, "realloc");
	sbp->segs = segl;
	sbp->buf = buf;
	...
	for(i=0; i< num_blocks; i++) {
		if(tba[i].bi_daddr
		   ...
		   && daddrtosn(tba[i].bi_daddr) == id)
		{
			syslog(LOG_ERR, "bi_daddr = 0x%x seg_addr = 0x%x; %p - %p = %ld",
				tba[i].bi_daddr, seg_addr,
				tba[i].bi_bp, seg_buf,
			...
			{
				tba[i].bi_bp = seg_buf + tba[i].bi_daddr - seg_addr;
			}
		}
	}
	...
	nba = (BLOCK_INFO *)realloc(sbp->ba, (sbp->nb + num_blocks) * sizeof(BLOCK_INFO));
	if (!nba)
		err(1, "realloc");
	sbp->ba = nba;
...
int clean_segments(FS_INFO *fsp, SEGS_AND_BLOCKS *sbp)
{
	...
	for (i = sbp->nb, bp = sbp->ba; i > 0; bp++, i--) {
		...
		if (bp->bi_lbn == -LFS_NDIR_BLOCKS)
			inino = ino;
		if (inino != ino && bp->bi_lbn > 0 && bp->bi_lbn > LFS_NDIR_BLOCKS) {
			++extra;
			inino = ino;
		}
	}
	ebytes =  LFS_BSIZE;
	ebytes += extra * LFS_BSIZE;
	if (debug) {
	...
	if (ebytes + nbytes > seg_size() * sbp->nsegs) {
		fork_coalesce(fsp);
	}
...
bi_tossold(const void *client, const void *a, const void *b)
{
	...
	return (((BLOCK_INFO *)a)->bi_daddr == LFS_UNUSED_DADDR ||
	    daddrtosn((((BLOCK_INFO *)a)->bi_daddr)) != t->seg);
}
